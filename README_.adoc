...
Here we find another problem in taking screenshot as file. *How can we store a screenshot into a file while retaining the metadata such as the source URL associated with the file?* My library https://github.com/kazurayam/materialstore[materialstore] can solve this problem.
